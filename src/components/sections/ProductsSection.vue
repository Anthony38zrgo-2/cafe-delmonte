  ...
  <section id="productos" class="section-space bg-beige" aria-labelledby="products-title">
    <div class="page-container">
      ...
      <div class="mt-12 grid gap-5 md:grid-cols-2">
        <article v-for="(product, index) in products" :key="product.id" class="group relative flex min-h-[28rem] flex-col overflow-hidden rounded-[2rem] border p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:p-9" :class="index === 1 ? 'border-gold bg-coffee text-warm-white' : 'border-roast/15 bg-warm-white text-coffee'">
          ...
          <dl class="mt-7 grid grid-cols-2 gap-x-4 gap-y-3 border-t pt-6 text-xs" :class="index === 1 ? 'border-warm-white/15 text-cream/85' : 'border-roast/15 text-roast/75'"><div><dt class="font-bold">Origen</dt><dd class="mt-1">Jaén - Cajamarca</dd></div><div><dt class="font-bold">Tueste</dt><dd class="mt-1">Medio</dd></div><div class="col-span-2"><dt class="font-bold">Presentación</dt><dd class="mt-1">En grano o molido según preferencia.</dd></div></dl>
          <div class="mt-auto flex flex-col items-start justify-between gap-5 pt-8 sm:flex-row sm:items-end"><p><span class="block text-[10px] font-bold uppercase tracking-widest opacity-60">Precio</span><strong class="mt-1 block font-display text-2xl">{{ product.price }}</strong></p><button type="button" class="btn-primary" :aria-label="`Configurar pedido de ${product.name}`" @click="openOrder(product, $event)">Pedir por WhatsApp <span aria-hidden="true">↗</span></button></div>
        </article>
      </div>
    </div>
